export default function render(data) {
    const templEl = document.querySelector('#event-card-template');
    ...
    data.forEach(item => {
        ...
        if (isCritical) {
            ...
            const closeBtn = instance.querySelector('.event-card__close');
        ...
        if (item.data && item.data.albumcover) {
            const albumCover = music.querySelector('.music__albumcover');
        ...
        if (item.data && item.data.image) {
            const view = camera.querySelector('.event-card__view');
